...
from .utils import scale_list, unit_list, get_surfaceform_sim, sort_cells
...
class Document:
    ...
    def get_text_text_edge_list(self):
        ...
        for i  in range(len(self.mentions)):
            for j in range(i+1, len(self.mentions)):
                ...
                proximity = 1.0/distance # use the inverse of the distnace, dont use the text length as the value will be more than one
                # compute the surface form and the value similarity for the graph and ILP
                surface_form_sim = get_surfaceform_sim(mention1.surface_form, mention2.surface_form)
    ...
    def track_solution(self, mention):
        #print("track mention", mention.surface_form)
        sentence_start, sentence_end = mention.get_sentence_span()
        #print("sentence to track", sentence_start)
        solution = mention.solution
        ...
        cells = cells.split(',')
        #print("table, cells", table,cells)
        if table in self.summary_tracker:
            table_tracker = self.summary_tracker[table]
        else:
            table_tracker = {}
        coverage = len(cells)
        #print("coverage",coverage)
        if sentence_start in table_tracker:
            table_tracker[sentence_start] += coverage
        else:
            table_tracker[sentence_start] = coverage

        #print("table Tracker for sent", table_tracker[sentence_start])

        self.summary_tracker[table] = table_tracker

    def get_best_sentence(self, table, table_tracker):
        #print("best Sentence for table", table)
        if not table_tracker:
            retunr -1
        sorted_sentences = sorted(table_tracker.items(), key = lambda kv : kv[1], reverse=True)
        #print(sorted_sentences)
        return sorted_sentences[0][0]

    def get_summary_sentences(self):
        summary_sentences = []
        for table, table_tracker in self.summary_tracker.items():
            sentence = self.get_best_sentence(table, table_tracker)
            if sentence >=0:
                summary_sentences.append(sentence)

        #print(summary_sentences)
        return set(summary_sentences)


    def get_text_with_mentions_mark_sentences(self):
        summary_sentences = self.get_summary_sentences()
        text =[]
        related_tables =[]
        for sentence_span in self.sentences_span:
            start,end = sentence_span
            sentence,sentence_related_tables = self.get_sentence_with_mentions(start,end)
            if start in summary_sentences:
                sentence = "<mark style=\"background-color:yellow;\">" + sentence + "</mark>"
            text.append(sentence)
            related_tables.extend(sentence_related_tables)
        related_tables = set(related_tables)
        if len(related_tables)>0:
            print("related tables %s"%related_tables)
        return " ".join(text), related_tables

    def get_sentence_with_mentions(self, start, end):
        text = self.text[start:end]
        related_tables =[]
        btn_classes = ['btn-primary', 'btn-success','btn-info','btn-warning']

        html_link ="""<a href="#" class="btn {} btn-xs btn-popover"
         tabindex="0" data-toggle="popover" data-trigger="focus" 
         data-popover-content="{}" 
         table-cells="{}"
         confidence ="{:.2%}"
        data-placement="bottom" role="button" >{}</a>
        """
        additional_chars = 0

        for mention in self.mentions:
            mention_sent_start, mention_sent_end =  mention.get_sentence_span()
            if mention_sent_start != start:
                continue
            mention_text = mention.surface_form
            if mention.solution:
                table =int(mention.solution[0].split('.')[0])
                cells = mention.solution[0].split('.')[1]
                if len(cells.split(',')) > 1:
                    mention_text += '<sup>*</sup>'
                related_tables.append(table)
                #<a href=\"#/\" onclick=\"highlight_cells('{}','{}')\">{}</a>,
                table_div_id = '#doc-table-'+str(table)
                btn_class = btn_classes[table%len(btn_classes)]
                mention_text = html_link.format(btn_class, table_div_id, mention.solution[0], mention.solution[1], mention_text) 

            text = "%s[%s] %s"%(text[:additional_chars + mention.get_sentence_start_offset()],  mention_text, text[additional_chars+mention.get_sentence_end_offset()+1:])
            additional_chars = additional_chars +2 + len(mention_text) - len(mention.surface_form)

        #print("annotated sentence, related tables", text, related_tables)
        return text, related_tables

    def get_text_with_mentions(self):
        self.get_text_with_mentions_mark_sentences()
        btn_classes = ['btn-primary', 'btn-success','btn-info','btn-warning']
        ...
        for mention in self.mentions:
            mention_text = mention.surface_form
            if mention.solution:
                #add solution next to the mention 
                table =int(mention.solution[0].split('.')[0])
                cells = mention.solution[0].split('.')[1]
                if len(cells.split(',')) > 1:
                    mention_text += '<sup>*</sup>'
                ...
                btn_class = btn_classes[table%len(btn_classes)]
                mention_text = html_link.format(btn_class, table_div_id, mention.solution[0], mention.solution[1], mention_text) 
                #"""<span class="mention"> {}|<span class="mention-answer" name="{}.{}">{}</span>,{:.2%}</span>""".format(mention_text, table, cells, mention.solution[0], mention.solution[1])
            
            text = "%s[%s] %s"%(text[:additional_chars + mention.start_offset],  mention_text, text[additional_chars+mention.end_offset+1:])
            additional_chars = additional_chars +2 + len(mention_text) - len(mention.surface_form)
    ...
    def get_mention_scale_unit(self, mention):
        ...
        for word in tokens:
            if word in scale_list:
                scale = scale_list[word]
    ...
    def get_exact_single_cell_match(self, mention):
        ...
        for table in self.tables:
            for table_mention in table.mentions.values():
                ...
                if get_surfaceform_sim(mention.surface_form, table_mention.surface_form) > 0.90:
                    print("similar: ", mention.surface_form, table_mention.surface_form)
                    count +=1
        return count

        pass
    def get_mention_features_for_classification_as_list(self):
        lines = []
        ...
        for mention in self.mentions:
            ...
            line = mention.get_features_as_list(self.doc_id)
            if line:
                line.append(exact_match_count)
                lines.append(line)
