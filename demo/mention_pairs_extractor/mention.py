import regex as re
from .utils import get_scale_unit
class Mention:
...
#                self.document_id = document_id
        ...
        scale,unit = get_scale_unit(mention)
        self.unit = unit
        self.scale = scale
    ...
    def get_sentence_span(self):
        return self.sentence_span
    def get_sentence_start_offset(self):
        if self.sentence_span:
            return self.start_offset - self.sentence_span[0]  
        else:
            return self.start_offset
    def get_sentence_end_offset(self):
        if self.sentence_span:
            return self.end_offset - self.sentence_span[0]  
        else:
    ...
    def set_solution(self, target, probability):
        self.solution = (target, probability)
        sol = self.solution[0].split('.')
        table = sol[0]
        cells= sol[1]
        if len(sol) > 2:
            aggr = sol[2]
...
#                print("negative sign", mention, self.negative_sign)
    ...
    def get_features_as_list(self, doc_id):
        mention_id = self.get_full_id()
        if not self.features:
            return None
        desired_features_keys = [ "modifiers","precision", "scale", "unit", "aggregate_function","glbl_perc","glbl_sum","glbl_diff", "glbl_rat","lcl_perc", "lcl_sum","lcl_diff", "lcl_rat","glbl_stats","glbl_finance","glbl_unit","glbl_temp","lcl_stats","lcl_finance","lcl_unit","lcl_temp"]
        desired_features =[str(doc_id), str(mention_id)]
        for feature in desired_features_keys:
            desired_features.append(self.features[feature])
