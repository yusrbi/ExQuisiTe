...
from  .utils import get_word_list, isQuantity, get_surfaceform_sim, isYear, get_scale_unit, get_zero_based_scale
...
class Table(object):
    ...
    def get_table_table_edge_list(self, doc_id):
        ...
        for i in range(len(cells)):
            ...
            for j in range(i+1, len(cells)):
                ...
                surfaceform_sim = get_surfaceform_sim(mention1.surface_form, mention2.surface_form)
    ...
    def get_mentions(self):
        ...
        #print("all_mentions", all_mentions)
        return all_mentions
    ...
    def compute_sum_and_avg(self,key,  values):
        ...
        print('sum',sum_, key_lst,acc_unit)
...
#                print(self.html)
        ...
        for row in rows:
            ...
            for col in columns:
                ...
                if col_content:
                    ...
                    if isQuantity(col_content):
                        self.n_quantity_cells = self.n_quantity_cells +1
                    #else:
                    #save tokens
                    # add all to context 
                    self.general_context_tokens.extend(get_word_list(col_content))
    ...
    def extract_mentions(self):
        ...
        for i  in range(self.row_count):
            for j  in range(self.col_count):
                ...
                if indx in self.mentions:
                    ...
                    if isQuantity(col_val) and not (isYear(col_val)):
                        #print( indx )
            ...
            if len(self.row_context[i]) >0:
                scale,unit = get_scale_unit(self.matrix[i][0][1])
                if scale and scale != 1  or unit:
                    ...
                    scale = get_zero_based_scale(content)
        ...
        for col in range(self.col_count):
            if len(self.col_context[col]) > 0:
                scale,unit = get_scale_unit(self.matrix[0][col][1])
                if scale and scale != 1  or unit:
    ...
    def get_table_global_scale_unit(self):
        ...
        for capt in self.caption_tokens:
            scale,unit = get_scale_unit(capt)
            if scale and scale != 1:
                self.scale = scale
            if unit:
                self.unit = unit
        ...
        for footer in self.footer_tokens:
            scale,unit = get_scale_unit(footer)
            if scale and scale != 1:
                self.scale = scale
            if unit:
                self.unit = unit
