...
from  .sentence_processor  import stem, stopwords_lst
...
for key in keywords.keys():
    keywords[key] = set([stem(word.lower()) for word in keywords[key]])
...
def isYear(mention):
    ...
    if re.fullmatch(r'199\d|20[01]\d', mention):# years are not mentions
        return True
    return False
...
def get_zero_based_scale(string):
    if re.search(r"($000|\D000's|\D000s)", string):
        return pow(10,3)
    elif re.search(r"($000000|\D000000's|\D000000s)", string):    
        return pow(10,6)
    return 1
...
def get_scale_unit(string):# assume the string always lower case
    ...
    if 'million' in string  or 'mega' in string:
        ...
        scale = 100
    elif re.search(r"(\bcents\b|\bcent\b)", string):
        scale = 0.01
        unit = '$'  
    elif re.fullmatch(r'\p{Sc}\d+([.,]\d+)*bn?', string):
        scale = pow(10,9)
    elif re.fullmatch(r'\p{Sc}\d+([.,]\d+)*mn?', string):
        scale = pow(10,6)
    elif re.fullmatch(r'\p{Sc}\d+([.,]\d+)*k', string):
        scale = pow(10,3)
    elif re.fullmatch(r'\(?\p{Sc}?000\'?s(\somitted)?\)?', string):
        scale = pow(10,3)
    elif re.fullmatch(r'^\(?\p{Sc}?000\'?s.*', string):
        scale = pow(10,3)
