from  .utils import get_aggr_fnc_from_id, load_mention_classifications, load_priors_by_mention_id, write_list_as_tsv
import sys 
...
def filtered_priors_for_app(mention_pairs, text_mentions):
    k = 10
    filtered_priors =[]
    baseline_filtered ={}
    
    for doc_id in mention_pairs.doc_id.unique():
        priors = mention_pairs[mention_pairs['doc_id'] == doc_id]
        #print("init", priors)
        mention_ids = priors.mXId.unique()
        mention_class = text_mentions[text_mentions['doc_id'] == str(doc_id)]    
        for mention_id in mention_ids:
            m_priors = priors[priors['mXId'] == mention_id].as_matrix().tolist()
            m_class = np.asarray(mention_class[mention_class['m_id'] == str(mention_id)])[0]
            #print("selected priors", m_priors)
            filtered_ = adaptive_filtering(m_priors,m_class,  k)
            #print("filtered priors", filtered_)
            #TODO
            #print_conflicts(filtered_)
            filtered_priors.extend(filtered_)
            baseline_filtered[str(doc_id)+'.'+str(mention_id)] = filtered_
...
def main():
    ...
    k = int(sys.argv[3])
    priors = load_priors_by_mention_id(file_name)
    #TODO get base_line classification here!    
    print_base_line_results(priors)

    mention_class = load_mention_classifications("mention_classification_test_all")
    filtered_priors = start_filtering(priors, mention_class)
    write_list_as_tsv(output_name, filtered_priors)

def start_filtering(priors, mention_class):
    filtered_priors =[]
    baseline_filtered ={}
...
def print_base_line_results(all_priors):
    ...
    for mention, priors in all_priors.items():
        ...
        priors.sort(key = lambda prior: float(prior[-2]), reverse= True )  
        for i in range(len(priors)):
            answer = priors[i]
            cls = answer[cls_indx]
            aggr_fnc = get_aggr_fnc_from_id(answer[2])
            #print(cls)
...
def filter_mention(priors, k):
    ...
    priors.sort(key = lambda prior: float(prior[-1]), reverse= True ) #max(float(prior[4]), float(prior[5])), reverse = True) 
    answer = None
    count = 0
    top_k =[]
    mTid = 2
    ...
    for prior in priors:
        ...
        mx = prior[mx_idx]
        mt= prior[mt_idx]
        unit_match =prior[unit_idx]
        aggr_fn_match =prior[aggr_idx]
        prob = prior[-2] #-1 is the aggr_prob
        is_exact = False
        if mx == mt:
            is_exact = True
        if cls == 'X1':
        ...
        if count < k:
            ...
            if cls == 'X1':
                count_pos(prior[mTid], True, is_exact)
                #count_all(prior[3], True, is_exact)
            count_all(prior[mTid],True, is_exact)
            top_k.append(prior)
            count = count +1
        else:
            if cls == 'X1':
            ...
            #    print('\t'.join(prior))
                missed = missed + 1
                count_pos(prior[mTid], False, is_exact)
            count_all(prior[mTid], False, is_exact)
...
def prune(mention_pairs, mention_class):
    #print("class", mention_class)
    if not mention_class.any():
    ...
    mention_class[-5:] = [ float(x) for x in mention_class[-5:] ]
    prob = mention_class[-5:] 
    #print("Class Prop Not Sorted", prob)
    prob = enumerate(prob)
    
    #print("Prior", mention_pairs[0])
    prob = sorted(prob,key= lambda x:x[1],reverse=True)
    #print("probabilities sorted",prob)
    pred_mention_class = aggregates[prob[0][0]]
    ...
    for pair in mention_pairs:
        ...
        if mx == mt:
            is_exact = True
        aggr_fnc = get_aggr_fnc_from_id(pair[mTid])
        ...
        #         count_pos(pair[mTid], False, is_exact)
        #     count_all(pair[mTid], False, is_exact)
        #     continue
...
def get_mention_type(mention_pairs):
    ...
    mx_idx = 17
    mt_idx = 18
    mention_pairs.sort(key = lambda pair :float( pair[-2]), reverse = True)
    ...
    for pair in mention_pairs:
        ...
        if count > limit:
            break
        pair_type = get_pair_type(str(pair[mx_idx]), str(pair[mt_idx]))
...
def filter_mention_2(priors, k):
    ...
    for prior in priors:
        ...
        if count < k:
            if cls == 'X1':
                count_pos(prior[2], True, is_exact)
            top_k.append(prior)
            count = count +1
        #elif prob > 0.9:
        #    if cls == 'X1':
        ...
        #    count = count + 1
            ...
            #    print('\t'.join(prior))
            missed = missed + 1
            count_pos(prior[2], False, is_exact)
            break
    #top_k = priors[:min(k, len(priors))]
