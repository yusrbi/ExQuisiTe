...
from .utils import write, read_tsv_files, read_tsv_as_dictionary
import codecs
import glob
import os
import numpy as np
...
beta = 0.7 # prior
#threshold = 0.0 # for selecting True relation
count_global_fn =0
count_global_fn2=0
lambda1 =0.9 # weight
...
def read_edges(file_name):
    return read_tsv_as_dictionary(file_name, key =0)
...
def solve_document_rwr( priors, edges, document, threshold):
    global alpha, beta
    ...
    for mention in mentions:
        ...
        if candidates.shape[0] ==1:
            ...
            print("Multiple Candidates, Resolve Using RWR")
            remove_mention_candidates_edges(st_graph,str(mention.mention_id))
            pers_dict = get_personalized_dict(st_graph.nodes(), str(mention.mention_id))
...
def solve_batch(path, all_priors):
        ...
        all_edges = read_tsv_files(files, key =0)
        mapping_results =[]
...
def load_priors(file_name):
    return read_tsv_as_dictionary(file_name, key =1)
