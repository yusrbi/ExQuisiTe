...
MONEY_COMPLEX =r'\p{Sc}\d+([\,\.]\d+)*(\–|\-|\−|\s?\±\s?)\p{Sc}\d+([\,\.]\d+)*((\s[mM]illions?|\s[tT]housands?|\s[hH]undereds?|\s[bB]illions?|\s?[Tt]|\s?[Mm]n?|\s?[Kk]|\s?[Bb]n?)\b)?'
#ex. 12% 12 per cent 12 percent
PERCENT_REGEX = r'\b\d+(\.\d{1,3})?(\%|\s?[Pp]er\s?cent)'
#ex. 12 to 13 percent , 12-13%
PERCENT_COMPLEX = r'[\+\-\s]\d{1,3}(\.\d{1,2})?(\–|\-|\−|\s?\±\s?)[\+\-]?\d+(\.\d{1,2})?(\%|\s?[Pp]er\s?cent)'
#ex. January 12, 2015
...
COMPLEX_UNIT_0 =r'[\+\-\–]?\d+([\,\.]\d+)?(\s?[^\s\d]+(\sper\s|\s?\/\s?)[^\s]+)(\s?(\–|\-|\−)\s?[\+\-\–]?\d+([\,\.]\d+)?)\s?[^\s\d]+(\sper\s|\s?\/\s?)[^\s]+'
#ex. + 123 KM per hour , 100km/hr, 100 to 5 km/hr
COMPLEX_UNIT=r'[\+\-\–]?\d+([\,\.]\d+)?(\s?(\–|\-|\−)\s?[\+\-\–]?\d+([\,\.]\d+)?)?\s?[^\s\d]+(\sper\s|\s?\/\s?)[^\s]+'
#10 x 100 km/hr
COMPLEX_UNIT_2=r'[\+\-\–]?\d+([\,\.]\d+)?\s?(\×|\±)\s?[\+\-\–]?\d+([\,\.]\d+)?\s?[^\s\d]+(\sper\s|\s?\/\s?)[^\s]+'
#ex. 100 to 123 users , 123 items
COMPLEX_UNIT_3=r'\b\d+([\,\.]\d+)*((\–|\-|\−|\s?\×\s?|\s?\±\s?)\d+([\,\.]\d+)*)?(\s[mM]illions?|\s[tT]housands?|\s[hH]undereds?|\s[bB]illions?|\s?[Tt]|\s?[Mm]n?|\s?[Kk]|\s?[Bb]n?|\s[Kk]ilos?)?\b'
REF_REGEX = r'\[\d+\]'
