...
import nltk.data
import nltk
from .sentence_processor import get_sentences_spans, extract_noun_phrases, fast_extract_noun_phrases
from . import utils
...
class Document:
    ...
    aggregate_kw = set (['total','overall','sum','average','decrease','increase','percent', 'percentage','difference'])
    aggregate_kw_to_function ={'total':'sum','overall':'sum','sum':'sum','average':'avg','decrease':'rat','increase':'rat','percent':'percentage', 'percentage':'percentage','difference':'dif'}
    MAX_TABLES = 10
    ...
    def get_text_with_mentions(self):
        ...
        for mention in self.mentions:
            ...
            if mention.solution:
                ...
                {:.2%}""".format(mention_text, table, cells, mention.solution[0], mention.solution[1])
    ...
    def add_mention(self, mention_id, mention, start_offset, end_offset):
        ...
        mention = Mention(mention_id, mention, start_offset,end_offset)
    ...
    def get_mention_modifiers(self, mention):
        ...
        modifier = 'None'
        for word in reversed(sentence_before.split(' ')):

            if word in Document.approximation_kw:
                modifier = 'approx'
                break
            elif word in Document.exact_kw:
                modifier = 'exact'
                break
            elif word in Document.bounds_kw:
                modifier = 'bound'
                break
            elif word in Document.aggregate_kw:
                mention.set_aggregate_kw(Document.aggregate_kw_to_function[word])
        return modifier
