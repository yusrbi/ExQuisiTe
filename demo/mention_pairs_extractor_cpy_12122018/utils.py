...
stop_words = sentence_processor.stopwords_lst #load_stop_words('hdfs:///user/yibrahim/stopwords.txt') on the cluster you need to use sc.textFile('hdfs:///user/vagrant/1970-Nixon.txt')
...
def isQuantity(mention):
    ...
    if re.fullmatch(r'\(?[+-]?\d+([., ]\d+)*\)?.*',mention): # accept a number followed by  a word, it might be a unit
        return True
    if re.fullmatch(r'.*\(?[+-]?\d+([., ]\d+)*\)?',mention): # accept a number proceeded by a word, it might be a unit
        return True
    if re.fullmatch(r'.{1,20}\(?[+-]?\d+([., ]\d+)*\)?.{1,20}',mention): # accept a number proceeded or followed by  a word only 20 characters, it might be a unit
        return True
    else:
        return False
