...
class MentionPair:
    ...
    def process(self,text_mention, table_mention):
        ...
        self.surface_form_sim  = utils.get_surfaceform_sim(text_mention.surface_form, table_mention.surface_form)
    ...
    def get_approx_sim(self):
        if not self.sim:
            ...
            sims = sims+self.unit_sim + self.surface_form_sim+ self.aggr_func_sim
    ...
    def get_as_csv_line(self):
        ...
        line = '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%d\n'%(
            ids, self.val_rel_diff_no_scale , self.val_rel_diff_max, self.val_rel_diff_sum, self.scale_diff,self.precision_diff,
            self.unit_sim, self.modifier,self.aggr_func_sim,self.tokens_overlap, self.np_sim,self.global_tokens_overlap,self.global_np_sim,self.surface_form_sim,self.get_approx_sim(),self.text_mention.get_value(), self.table_mention.get_value(),self.text_mention.features['scale'], self.table_mention.features['scale'],self.get_ground_truth())
        #mXId\tmTid\tdiff\tdiff_max\tdif_sum\tscale\tprec\tunit\tmod\taggr\ttokn\tnps\gtokns\gnps\tsurfaceform\tGT

        return line
    def get_as_list(self):
        
        return [self.doc_id, self.text_mention.get_full_id(), self.table_mention.get_full_id(), self.val_rel_diff_no_scale,
                self.val_rel_diff_max, self.val_rel_diff_sum, self.scale_diff,self.precision_diff,
            self.unit_sim, self.modifier,self.aggr_func_sim,self.tokens_overlap, self.np_sim,self.global_tokens_overlap,
