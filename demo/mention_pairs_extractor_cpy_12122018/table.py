...
class Table(object):
    ...
    def get_table_table_edge_list(self, doc_id):
        ...
        #get all the cells
        cells = list(self.cell_indx_to_matrix_ij.keys())
        ...
        for cells, aggr_mentions in self.aggregate_mentions.items():
            cells = cells.split(',')
            #print("cells%s"%cells)
    ...
    def get_all_combinations(self, values):
        ...
        for i in range(n):
            for j in range(i+1,n):
                ...
                if (unit1 and unit2 and unit1 == unit2) or (not unit1 or not unit2) :#only combine if they are of the same unit, will work if both have none unit
                    combinations.append([values[i], values[j]])
                    #for k in range(j+1,n):
                     #   _,_,unit3,_ = values[k]
                      #  if(unit3 and unit1 and unit2 and unit3 == unit1) or (not unit3 or not unit1 or not unit2):
    ...
    def compute_aggregates_for(self, subset):
        for values in subset:
            key = self.get_key(values)
            if key  in self.aggregate_mentions:
                continue
            self.aggregate_mentions[key] = []
        # compute sum and average
            self.compute_sum_and_avg(key, values)
            if len(values) > 2:
                continue
        #compute diff
            self.compute_diff(key,values)
        #compute change ratio
            self.compute_ratio(key, values)
        # compute percentage
            self.compute_percent(key, values)
    ...
    def compute_percent(self,key, values):
        ...
        mention.add_aggregate_inverse(inverse)
        mention.precesion = max(prec1,prec2) # the max os the 2 quantities precision
        mention.unit='%'
    ...
    def compute_ratio(self,key, values):
        ...
        if value1 ==0 or value2 == 0:
            return
        ratio = 100*abs(value1-value2)/value2#"(%s-%s)/%s"%(value1,value2,value2) #(value1-value2)/value2
        mention = Mention(key, str(ratio),0,0)
        mention.set_table_id(self.table_id)
        mention.add_aggregate('rat',[indx1,indx2],unit1)
        inverse = 100*abs(value2-value1)/value1#"(%s-%s)/%s"%(value2,value1,value1) #(value2-value1)/value1
        mention.add_aggregate_inverse(inverse)
        mention.precision = max(prec1,prec2)
        mention.unit ='%'
        self.aggregate_mentions[key].append(mention)
